...
from agents.ollama_rag_agent import create_ollama_rag_agent, run_ollama_rag_agent, convert_chat_history
...
def main():
    ...
    with st.sidebar:
        ...
        model_path = st.text_input(
            ...
            value=settings.DEFAULT_MODEL_PATH,
            help="Contoh: C:/models/Llama-3.1-8B-Instruct.Q4_K_M.gguf"
        )
        n_gpu_layers = st.number_input("Lapisan GPU (n_gpu_layers)", min_value=-1, value=settings.DEFAULT_N_GPU_LAYERS, help="-1 untuk offload semua lapisan yang memungkinkan.")
        n_ctx = st.number_input("Ukuran Konteks (n_ctx)", min_value=512, value=settings.DEFAULT_N_CTX)
        ...
        embedding_model_name = st.selectbox(
            "Model Embedding",
            options=list(settings.AVAILABLE_EMBEDDING_MODELS.keys()),
            index=list(settings.AVAILABLE_EMBEDDING_MODELS.keys()).index(settings.DEFAULT_EMBEDDING_MODEL)
        )
        ...
        if st.button("Proses Dokumen"):
            if uploaded_files:
                st.session_state.vectorstore = process_documents(uploaded_files, embedding_model)
